...
`scripts/loop_review.py` holds the loop state, not your memory: scope, brief, passes, validation and findings live in `.loop-review/` inside the reviewed repository, and `accept` refuses until the exit condition holds. Use it at every step. Run it by its path **inside this skill's directory** — `scripts/…` relative to the reviewed repository may not exist, or may be a different copy. If an earlier loop is still active the first command refuses: finish it, or `reset` it once the user agrees it is dead (`--project` discards an abandoned ledger too; `--force` only to reset out of a live area, the user's call). Mention `.loop-review/` in the report so the user can gitignore or remove it.
